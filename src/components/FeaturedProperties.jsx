...
import { BhutanMapView } from './BhutanMapView';
import { 
  Bed, 
  Bath, 
  Maximize2, 
  ...
  return (
    <section id="properties-section" className="py-12 sm:py-16 px-4 sm:px-8 bg-white border-b border-stone-200/80">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Section Header with Map View Toggle */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 sm:mb-8">
          ...
          <div className="flex items-center gap-2.5">
            {/* Grid vs Map Toggle Switcher */}
            <div className="flex items-center bg-stone-100 p-1 rounded-xl border border-stone-200 text-xs font-bold text-slate-700">
              <button
                onClick={() => setViewMode('grid')}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg transition-all cursor-pointer ${
                  viewMode === 'grid'
                    ? 'bg-white text-[#9e1b27] shadow-xs'
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg transition-all cursor-pointer ${
                  viewMode === 'map'
                    ...
                    : 'text-slate-600 hover:text-[#9e1b27]'
                }`}
              >
                <Map className="w-3.5 h-3.5" />
                <span>🗺️ Bhutan Map</span>
              </button>
            </div>

            <button 
              onClick={() => openDetail(properties[0], 'property')}
              className="hidden sm:flex text-xs sm:text-sm font-bold text-[#9e1b27] hover:text-[#80131d] items-center gap-1 hover:underline cursor-pointer ml-2"
            >
              <span>View All</span>
              <span>&gt;</span>
            </button>
        ...
        {/* Render Map View or Carousel Track */}
        {viewMode === 'map' ? (
          <BhutanMapView />
        ) : (
          <div className="relative group">
            ...
            >
              {properties.map((prop) => {
                ...
                const isComparing = compareList.some(i => i.id === prop.id);
